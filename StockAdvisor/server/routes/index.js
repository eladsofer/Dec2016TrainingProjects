var elasticSearch = require('elasticsearch');

var express = require('express');

var loremIpsum = "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.";

var users = [
  {id: 201, firstName: "Dean",lastName: "kiverkian", nickName: "yerm", Age: 35, aboutMe: "I love stocks", joinDate: "13/12/2014",reviews:5, score:36, profilePicUrl:"https://s-media-cache-ak0.pinimg.com/564x/2f/69/bf/2f69bf2bbee52478bf19e27afb134a12.jpg"},
  {id: 202, firstName: "Sharon",lastName: "Levi", nickName: "shash", Age: 30, aboutMe: "I love stocks", joinDate: "13/12/2014",reviews:5, score:36, profilePicUrl:"data:image/jpeg;base64,"},
  {id: 203, firstName: "Roni",lastName: "Dalumi", nickName: "rond", Age: 25, aboutMe: "I love stocks", joinDate: "13/12/2014",reviews:5, score:36, profilePicUrl:"https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcQZ_j2Rf4KrdM1VxZasaeKScN6_zGkZkxKFda_lmHYtwyWTWyeDYw"},
  {id: 204, firstName: "Rami",lastName: "Dill", nickName: "dillr", Age: 20, aboutMe: "I love stocks", joinDate: "13/12/2014",reviews:5, score:36, profilePicUrl:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQVXbMsm1QBZNGpbjGKFTQ3isdEcpUtCkMBDvtKIuwYBL_oQ1gY"},
  {id: 204, firstName: "Yosifun",lastName: "Kashmiri", nickName: "yoss", Age: 20, aboutMe: "I love stocks", joinDate: "13/12/2014",reviews:5, score:36, profilePicUrl:"https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcS5wc-GtFkfEs8N5nREqysIeGod79rszRKoKoczqOiJXjn8HUD2Rg"}
];

var stocks = [
  {id: 629014, name: "טבע",symbol: "טבע", description: "Pharmacy", currentPrice: 14280, sector: "Private"},
  {id: 373019, name: "אאורה",symbol: "אאורה", description: "מניות", currentPrice: 195.50,sector: "מניות"},
  {id: 1083443, name: "אינטרנט זהב", symbol: "אנזהב", description: "אינטרנט זה אנחנו", currentPrice: 4400, sector: "מניות"}
];



// Promise.resolve()
//   .then(() => getUserById(1))
//   .then(function(res)
//   {
//     console.log(res);
//   })
//   .catch(function(err) {
//     console.log("ERROR", err);
//   });

var router = express.Router();

/* GET home page. */
router.get('/', function(req, res, next) {
  res.render('index', { title: 'Express' });
});


router.get('/stocks', function(req, res, next) {
  res.json(stocks);
});


router.get('/stock/:id', function(req, res, next) {
  var stock = stocks.filter(function(a){ return a.id == req.params.id })[0];
  res.json(stock);
});


router.get('/users', function(req, res, next) {
  res.json(users);
});



router.get('/user/:id', function(req, res, next) {

  getUserById(req.params.id)
    .then(function(resUser )
    {
      var user = {
        "id": resUser._id,
        "firstName": resUser._source.first_Name,
        "lastName": resUser._source.last_Name,
        "nickName": resUser._source.nick_Name,
        "Age": resUser._source.Age,
        "aboutMe": resUser._source.about_Me,
        "joinDate": resUser._source.join_Date,
        "profilePicUrl": resUser._source.profile_Pic_Url
      } ;
      console.log(user);
      res.json(user);
    })
    .catch(function(err) {
      console.log("ERROR", err);
    });

  //var user = users.filter(function(a){ return a.id == req.params.id })[0];

  //res.json(user);
});

function getUserById(id) {

  var client = new elasticSearch.Client({host: '192.168.100.38:9200'});

  return client.get({
    index: 'stock_advisor',
    type: 'user',
    id: id
  }).then(function (response) {
    //console.log(response);
    return response;
  });

}

router.get('/user_reviews/:id', function(req, res, next) {
  var reviews = [
    {id: 100, postDate: "13/12/2014",content: loremIpsum, owner: users[0]},
    {id: 101, postDate: "10/12/2014",content: loremIpsum, owner: users[0]},
    {id: 102, postDate: "8/12/2014",content: loremIpsum, owner: users[0]}
  ];

  var review = reviews.filter(function(a){ return a.id == req.params.id })[0];
  res.json(review);
});
/*
 id : number,
 name : string,
 symbol : string,
 description : string,
 currentPrice : number,
 sector : string // TODO: Enum
 */
router.get('/user/reviews/:userId', function(req, res, next) {
  var reviews = [
    {id: 100, postDate: "13/12/2014",content: loremIpsum, owner: users[0], stock: { id: 629014, name: "טבע", description: "Pharmacy", currentPrice: 14280, sector: "Private" }},
    {id: 101, postDate: "10/12/2014",content: loremIpsum, owner: users[0]},
    {id: 102, postDate: "8/12/2014",content: loremIpsum, owner: users[0]}
  ];
  var filteredReviews = reviews.filter(function(a){ return a.owner.id == req.params.userId });

  res.json(reviews);
});

router.get('*',function(req,res){
  res.sendfile('./404.html');
});

module.exports = router;
